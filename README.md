...
├── assets/                    # 应用资源（图标等）
├── imgs/                      # README 应用截图
├── scripts/                   # 数据抓取（fetch_api_data.py）、Android 版本同步与签名
...
├── android-build.bat          # Android 调试构建
├── android-release.bat        # Android 发布构建（签名 APK）
...
除 Windows 桌面端外，本项目也构建 Android 应用（aarch64 APK）。需自行配置 `JAVA_HOME` / `ANDROID_HOME` / `NDK_HOME`，详见 `android-build.bat` / `android-release.bat`。
...
构建前由 `scripts/sync-android-version.mjs` 从 `src-tauri/Cargo.toml` 同步版本号，构建后由 `scripts/sign-apk.mjs` 完成签名。
...
| `npm run tauri:android` | 构建并自动签名 aarch64 Android APK |
| `npm run tauri:android:dev` | 真机 / 模拟器开发调试 |
...
仓库配置了 `.github/workflows/release.yml`：推送 `v<version>` 格式的 tag 后，在 Windows runner 上构建 NSIS 安装包并自动创建 GitHub Release（说明正文取自 `RELEASE_NOTES.md`），随后在 Ubuntu runner 上构建 aarch64 Android APK 并追加到同一 Release；也可在 Actions 面板手动触发。具体发布流程见 AGENTS.md。
